...
Gui::Gui() :
		_pGuiObserver(nullptr), _redLedOn(false), _xAxisPixelsPerField(0) {
...
void Gui::initialize() {
	...
	// Some custom drawing options
	{
		...
		_xAxisPixelsPerField = gwinGetWidth(ghGraph)
				/ observer().getTDivCount();
		GGraphStyle graphStyle = { { GGRAPH_POINT_DOT, 3, GFX_WHITE },  // Point
...
void Gui::drawGraphPoints(uint16_t * values, uint16_t count,
		float xScale /* = 1 */) {
	const coord_t MAX_WIDTH = gwinGetWidth(ghGraph);
	const coord_t MAX_HEIGHT = gwinGetHeight(ghGraph);
	...
	if (ghGraph) {
		...
		if (points) {
			...
			for (uint32_t i = 0; i < MAX; i++) {
				...
				if (isRedLedEnabled()) {
					if(val > threshold || startValueFound)
					{
						startValueFound = true;
						points[i-nValIgnored].x = i * xScale;
					...
					points[i].x = i * xScale;
					points[i].y = val * MAX_HEIGHT / 4096;      // Scaling
				}
...
void Gui::createTouchThread() {
	gfxThreadCreate(touchThread, TOUCH_THREAD_STACK_SIZE,
			3 /*makeFreeRtosPriority(osPriorityNormal)*/, &touchThreadHandler,
			this);
}
